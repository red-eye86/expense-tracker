...
const defaultInvestments = [
    { sn: 1, inv: "Equity Mutual Fund", plan: "Sample Index Fund", status: "Active", amount: "10,000/month", start: "01-01-26", dop: "1st", platform: "Broker A", num: "1234567890", app: "APP123", policy: "POL123", bank: "Bank Account 1" },
    { sn: 2, inv: "Health Insurance", plan: "Sample Health Plan", status: "Active", amount: "20,000/year", start: "15-06-25", dop: "", platform: "Broker B", num: "0987654321", app: "", policy: "HLTH456", bank: "Bank Account 2" },
    { sn: 3, inv: "Future Goal", plan: "Education Fund", status: "Future", amount: "5,000/month", start: "", dop: "", platform: "", num: "", app: "", policy: "", bank: "" }
];
...
// The payment will then appear automatically in Household and the respective Profile tab,
...
let dynamicProfiles = JSON.parse(localStorage.getItem('dynamicProfiles')) || [
    { id: 'tab-profile1', name: 'Primary Profile', accounts: ['Bank Account 1', 'Bank Account 2', 'Credit Card 1'] },
    { id: 'tab-profile2', name: 'Secondary Profile', accounts: ['Bank Account 3', 'Bank Account 4'] }
];
...
function saveDynamicEmis() {
    ...
    fetch('api.php?action=sync_emis', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(dynamicEmis)
    }).catch(err => console.error("Failed to sync EMIs", err));
}
...
const DEFAULT_TYPES = ['Expense', 'Income', 'Transfer', 'Onhand', 'Historical'];
...
const DEFAULT_ACCOUNTS = [
    'Bank Account 1', 'Bank Account 2', 'Credit Card 1',
    'Bank Account 3', 'Bank Account 4'
];
...
function renderTransactions() {
    ...
    // Dynamically grab tbodies for all profiles
    const profileTbodies = {};
    dynamicProfiles.forEach(p => {
        profileTbodies[p.id] = document.getElementById(`tx-list-${p.id}`);
        if(profileTbodies[p.id]) profileTbodies[p.id].innerHTML = '';
    });
    ...
    getFilteredTransactions().forEach((t) => {
        ...
        // Determine which profile owns the 'from' account
        let fromProfile = dynamicProfiles.find(p => p.accounts.includes(t.account));
        let toProfile = t.toAccount ? dynamicProfiles.find(p => p.accounts.includes(t.toAccount)) : null;
        ...
        if (t.type === 'Transfer') {
            ...
            if (fromProfile && profileTbodies[fromProfile.id]) {
                profileTbodies[fromProfile.id].innerHTML += rowHtml(currency, t.amount, 'active', 'Sent', true);
            }

            // Add to Receiver's Table
            if (toProfile && profileTbodies[toProfile.id]) {
                profileTbodies[toProfile.id].innerHTML += rowHtml(toCurr, convertedAmt, 'future', 'Received', false);
            }
        } else {
            if (fromProfile && profileTbodies[fromProfile.id]) {
                profileTbodies[fromProfile.id].innerHTML += rowHtml(currency, t.amount, badgeClass, t.type, false);
            }
        }
    });
}
...
async function generateProfessionalExcel(txs) {
    ...
    const maxExp = expTxs.length ? Math.max(...expTxs.map(t => Number(t.amount))) : 0;
    ws1.getCell(`K${cRIdx}`).value = maxExp; ws1.getCell(`K${cRIdx}`).border = borderAll; ws1.getCell(`K${cRIdx}`).numFmt = '#,##0.00'; ws1.getCell(`K${cRIdx}`).font = {bold:true};
    cRIdx++;
    ws1.getCell(`J${cRIdx}`).value = 'Smallest Expense'; ws1.getCell(`J${cRIdx}`).border = borderAll;
    const minExp = expTxs.length ? Math.min(...expTxs.map(t => Number(t.amount))) : 0;
    ws1.getCell(`K${cRIdx}`).value = minExp; ws1.getCell(`K${cRIdx}`).border = borderAll; ws1.getCell(`K${cRIdx}`).numFmt = '#,##0.00'; ws1.getCell(`K${cRIdx}`).font = {bold:true};
    cRIdx++;
    ...
    let tr2 = Math.max(totalCr2, totalIcr) + 3;
